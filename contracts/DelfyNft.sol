...
abstract contract Ownable is Context {
    ...
    function pendinOwner() public view virtual returns (address) {
...
contract DelfyNFT is ERC721, Ownable {
    ...
    modifier onlyOwnerAndMinter(uint256 tokenId) {
        address owner = ERC721.ownerOf(tokenId);
        require(
            _msgSender() == owner && isRoyalty[_msgSender()][tokenId],
            "ERC721: only creator as the current owner"
        );
        _;
    }

    modifier isValidRoyalty(
        address payable[] memory addresses,
        uint256[] memory values
    ) {
        require(
            addresses.length == values.length,
            "DelfyNFT: unequal array length"
        );
        require(
            isValid(values),
            "DelfyNFT:royalty cannot be greater than 100%"
        );
        _;
    }
    ...
    function isValid(uint256[] memory values) internal pure returns (bool) {
        uint256 total;
    ...
    event RoyaltyUpdated(address minter, uint256 tokenId);
    ...
    function updateRoyalty(
        ...
        external
        onlyOwnerAndMinter(tokenId)
        isValidRoyalty(royaltyAddrs, royaltyShares)
    {
        royalties[tokenId] = royaltyAddrs;
        royaltyFees[tokenId] = royaltyShares;
        emit RoyaltyUpdated(_msgSender(), tokenId);
    }

    function registerNewMinter(address newMinter) external onlyMinter {
        _addMinter(payable(newMinter), _msgSender());
    }
    ...
    function _addMinter(address _minter, address payable _referrer) internal {
        ...
        referredBy[_minter] = _referrer;
        emit MinterAdded(_referrer, _minter);
    }

    function restrictMinter(address _minter) external onlyOwner {
        _restrictMinter(_msgSender(), _minter);
    }

    function _restrictMinter(address sender, address _minter) internal {
        require(isMinter[_minter], "minter exists");
        isMinter[_minter] = false;
        emit RestrictMinter(sender, _minter);
    }

    function restoreMinter(address minter) external onlyOwner {
        _restoreMinter(_msgSender(), minter);
    }

    function _restoreMinter(address sender, address _minter) internal {
        require(!isMinter[_minter], "minter exists");
        isMinter[_minter] = true;
        emit RestoreMinter(sender, _minter);
    }

    function updateMinterAddress(address newAddr) external {
        require(isMinter[_msgSender()], "Only existing minters");
        uint256 i;
        for (i = 1; i < totalSupply(); i++) {
            if (isRoyalty[_msgSender()][i]) {
                isRoyalty[newAddr][i] = true;
            }
        }
        referredBy[newAddr] = referredBy[_msgSender()];
